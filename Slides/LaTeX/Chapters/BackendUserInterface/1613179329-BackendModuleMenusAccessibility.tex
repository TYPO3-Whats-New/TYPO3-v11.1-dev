% ------------------------------------------------------------------------------
% TYPO3 Version 11.1 - What's New (English Version)
%
% @author	Michael Schams <schams.net>
% @license	Creative Commons BY-NC-SA 3.0
% @link		http://typo3.org/download/release-notes/whats-new/
% @language	English
% ------------------------------------------------------------------------------
% Feature | 92704 | Improve keyboard navigation for module menus

\begin{frame}[fragile]
	\frametitle{Backend User Interface}
	\framesubtitle{Backend Module Menus}

	Backend users can now navigate through the main module and the help menu
	by using the keyboard. This is ideal for screen readers or other assistive
	technology.
	\newline
	The implementation follows the ARIA Best Practices 1.1.

%	\begin{figure}
%		\includegraphics[width=0.9\linewidth]{BackendUserInterface/1613179329-BackendModuleMenusAccessibility.png}
%	\end{figure}

\end{frame}

% ------------------------------------------------------------------------------
